(function (global) {
  ...
  shared.WIDTH = 600;
  shared.HEIGHT = 880;
  shared.TARGET_FPS = 60;
  shared.FRAME_TIME = 1 / shared.TARGET_FPS;

  shared.POWERUP_TYPES = {
  ...
  shared.randomRange = function randomRange(min, max) {
    return Math.random() * (max - min) + min;
  };

  shared.intersects = function intersects(a, b) {
    return (
      a.x < b.x + b.width &&
      a.x + a.width > b.x &&
      a.y < b.y + b.height &&
  ...
  shared.segmentCircleIntersects = function segmentCircleIntersects(
    x1,
    y1,
    x2,
    y2,
  ...
  shared.bulletAsteroidHit = function bulletAsteroidHit(bullet, asteroid) {
    ...
    return shared.segmentCircleIntersects(start.x, start.y, end.x, end.y, centerX, centerY, radius);
  };

  SpaceVoid.shared = shared;
})(window);
